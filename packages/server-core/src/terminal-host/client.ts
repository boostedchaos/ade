...
/**
 * Host-app hook: where the built daemon script (terminal-host.js) lives.
 * Electron registers app.getAppPath()/dist/main/terminal-host.js; the
 * headless ade-server registers its own dist path.
 */
let daemonScriptPathResolver: (() => string) | null = null;

export function setDaemonScriptPathResolver(resolver: () => string): void {
	daemonScriptPathResolver = resolver;
}

/**
 * Optional host-app hook: full argv (after process.execPath) used to spawn
 * the daemon. Defaults to [scriptPath]. ade-server uses this to inject
 ...
 * to process.execPath (Electron-as-node in the desktop app). ade-server
...
const socketMayExist = (): boolean =>
	isNamedPipePath(SOCKET_PATH) || existsSync(SOCKET_PATH);
...
export class TerminalHostClient extends EventEmitter {
	...
	/**
	 * Get path to daemon script (resolved by the host app — see
	 * setDaemonScriptPathResolver; Electron and ade-server lay out their
	 * built daemon differently).
	 */
	private getDaemonScriptPath(): string {
		if (!daemonScriptPathResolver) {
			throw new Error(
				"terminal-host client: no daemon script path resolver registered. " +
					"Call setDaemonScriptPathResolver() before spawning the daemon.",
			);
		}
		return daemonScriptPathResolver();
	}
	...
	private async waitForDaemon(): Promise<void> {
		...
		while (Date.now() - startTime < SPAWN_WAIT_MS) {
			if (isNamedPipePath(SOCKET_PATH)) {
				// Pipes have no filesystem presence — probe with a real connect.
