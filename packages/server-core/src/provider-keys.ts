...
import { getSecretStore } from "./secret-store-host";

/**
 * Local-first, single-user storage for provider API keys.
 *
 * Keys are encrypted via the host-provided SecretStore (Electron safeStorage
 * on the desktop, an AES-256-GCM file key on ade-server) and the
 * resulting blob is persisted, base64-encoded, in the local sqlite settings
 * row (settings.providerApiKeys, keyed by provider id). Plaintext keys never
 * touch disk and are only ever decrypted server-side — never returned to the
...
export function setProviderKey(provider: ProviderId, key: string): void {
	...
	const encrypted = store.encryptString(trimmed).toString("base64");
...
export function getProviderKey(provider: ProviderId): string | null {
	...
	try {
		return store.decryptString(Buffer.from(blob, "base64"));
	} catch {
		return null;
	}
}
