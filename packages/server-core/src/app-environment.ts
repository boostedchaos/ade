...
/**
 * One-time migration from the pre-ADE data dir: ~/.ade[-<ws>] → ~/.ade[-<ws>].
 * Only fires when the target doesn't exist yet and the dir wasn't overridden via env.
 */
function migrateLegacyHomeDir(): void {
	if (process.env[ADE_HOME_DIR_ENV] !== join(homedir(), SUPERSET_DIR_NAME))
		return;
	const legacyDir = join(
		homedir(),
		basename(SUPERSET_HOME_DIR).replace(/^\.ade/, ".ade"),
	);
